...
def randomly_sample_reads(input_file, sample_file,num_sample_reads, chunk_size, eps):
	...
	zero_counts = np.zeros(len(input_str)) #number of zeros received at each pos
	unique_reads = set([])
	...
	for i in range(num_sample_reads):
		...
		zero_counts[read_id*chunk_size:(read_id+1)*chunk_size] += (((input_data[read_id*chunk_size:(read_id+1)*chunk_size]+(np.random.rand(chunk_size) < eps))%2) == 0) 
...
def main():
    ...
    randomly_sample_reads( config.output_file, config.sample_file, num_samples, config.chunk_length, config.eps);
