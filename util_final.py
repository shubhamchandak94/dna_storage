...
def remove_index(num_oligos, BCH_bits, infile_name, outfile_data, outfile_index, bin_index_len, attempt_indel_cor=False, max_bitflips_sub=None, max_bitflips_indel=0):
    '''
    ...
    max_bitflips_sub: decoding of substitutions succeeds as long as number of bitflips is below this (default: BCH_bits). Setting this to 0 and setting attempt_indel_cor=False means that we are using BCH as an error detection code.
    max_bitflips_indel: decoding of indel case succeeds as long as number of bitflips is below this (default: 0). If this is too high, we will most likely see more than 1 success so we'll fail at the end.
    '''
    if max_bitflips_sub == None:
